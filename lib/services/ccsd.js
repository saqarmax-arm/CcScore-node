...
var bitcore = require('ccscore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var CcSRPC = require('ccsd-rpc');
...
/**
 * Provides a friendly event driven API to ccsd in Node.js. Manages starting and
 * stopping ccsd as a child process for application support, as well as connecting
 * to multiple ccsd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function CcS(options) {
  if (!(this instanceof CcS)) {
    return new CcS(options);
  }
...
util.inherits(CcS, Service);

CcS.dependencies = [];

CcS.DEFAULT_MAX_TXIDS = 1000;
CcS.DEFAULT_MAX_HISTORY = 50;
CcS.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
CcS.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
CcS.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
CcS.DEFAULT_SPAWN_RESTART_TIME = 5000;
CcS.DEFAULT_SPAWN_STOP_TIME = 10000;
CcS.DEFAULT_TRY_ALL_INTERVAL = 1000;
CcS.DEFAULT_REINDEX_INTERVAL = 10000;
CcS.DEFAULT_START_RETRY_INTERVAL = 5000;
CcS.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
CcS.DEFAULT_TRANSACTION_CONCURRENCY = 5;
CcS.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
CcS.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || CcS.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || CcS.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || CcS.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || CcS.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || CcS.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || CcS.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || CcS.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || CcS.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || CcS.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || CcS.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

CcS.prototype._initCaches = function() {
...
CcS.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
CcS.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
CcS.prototype.getPublishEvents = function() {
  return [
    {
...
CcS.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'ccsd/' + name, 'total:', this.subscriptions[name].length);
};

CcS.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'ccsd/' + name, 'total:', this.subscriptions[name].length);
};


CcS.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
CcS.prototype.unsubscribeBalance = function(emitter, addresses) {
...
CcS.prototype.unsubscribeBalanceAll = function(emitter) {
...
CcS.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'ccsd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

CcS.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'ccsd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
CcS.prototype.unsubscribeAddressAll = function(emitter) {
...
CcS.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = CcS.DEFAULT_CONFIG_SETTINGS;
...
CcS.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
CcS.prototype._expandRelativeDatadir = function() {
...
CcS.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in ccsd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in ccsd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in ccsd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './ccs.conf');

  log.info('Using ccs config file:', configPath);
...
CcS.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of ccscore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of ccscore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of ccscore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to ccsd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from ccsd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from ccsd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that ccsd is undergoing a reindex. ' +
             ...
             'of ccscore-node services will start.');
    node._reindex = true;
  }
};

CcS.prototype._resetCaches = function() {
...
CcS.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
CcS.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

CcS.prototype._initChain = function(callback) {
  var self = this;
...
CcS.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
CcS.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/ccs.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/ccs.conf';
    }
  }
  return networkPath;
};

CcS.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
CcS.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('ccsd/hashblock', message.toString('hex'));
    }
  }

};

CcS.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
CcS.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('CcS Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
CcS.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
CcS.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
CcS.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
CcS.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('ccsd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
CcS.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('ccsd/rawtransaction', message.toString('hex'));
    }
...
CcS.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || CcS.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

CcS.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
CcS.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
CcS.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('CcS Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || CcS.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

CcS.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
CcS.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/ccsd.pid';
...
CcS.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('CcS process unexpectedly exited with code:', code);
        log.warn('Restarting ccs child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('CcS process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn ccsd.'));
      }
...
CcS.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to ccsd.'));
    }
 ...
 */
CcS.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('CcS configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
CcS.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
CcS.prototype.syncPercentage = function(callback) {
  var self = this;
...
CcS.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, ccscore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
CcS.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
CcS.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
CcS.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
CcS.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in ccscore is most recent to less recent
 ...
 * @param {Function} callback
 */
CcS.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
CcS.prototype._getConfirmationsDetail = function(transaction) {
...
CcS.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
CcS.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
CcS.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A ccscoin transaction id
 * @param {Function} callback
 */
CcS.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
CcS.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
CcS.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
CcS.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
CcS.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
CcS.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
CcS.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
CcS.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
CcS.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
CcS.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
CcS.prototype.estimateSmartFee = function (blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a CcScore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
CcS.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getInfo = function(callback) {
...
CcS.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('ccsd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('ccsd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
CcS.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
CcS.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
      from = options && options.from ? options.from : '',
      gasLimit = options && options.gasLimit ? options.gasLimit : 0,
      amount = options && options.amount ? options.amount : 0;

  this.client.callContract(address, hash, from, gasLimit, amount, function (err, response) {
      if (err) {
          return callback(self._wrapRPCError(err));
      }
    callback(null, response.result);
 ...
 * @param {Function} callback
 */
CcS.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
CcS.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
CcS.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
CcS.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
CcS.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
CcS.prototype.getStakingInfo = function(callback) {
    var self = this,
