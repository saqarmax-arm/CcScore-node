...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'ccscore-lib': 'https://github.com/saqarmax-arm/ccscore-lib.git#master',
    'ccscore-node': 'https://github.com/saqarmax-arm/ccscore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/ccscore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the ccscore-node directory and configuration
