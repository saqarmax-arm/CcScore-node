...
var Transaction = ccscore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/ccs.conf')));
var CcSService = proxyquire('../../lib/services/ccsd', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultCcSConf = fs.readFileSync(path.resolve(__dirname, '../data/default.ccs.conf'), 'utf8');

describe('CcS Service', function() {
	...
	var baseConfig = {
		node: {
			network: ccscore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var ccsd = new CcSService(baseConfig);
			should.exist(ccsd);
		});
		it('will create an instance without `new`', function() {
			var ccsd = CcSService(baseConfig);
			should.exist(ccsd);
		});
		it('will init caches', function() {
			var ccsd = new CcSService(baseConfig);

			should.exist(ccsd.utxosCache);
			should.exist(ccsd.txidsCache);
			should.exist(ccsd.balanceCache);
			should.exist(ccsd.summaryCache);
			should.exist(ccsd.transactionDetailedCache);
			should.exist(ccsd.accountInfo);

			should.exist(ccsd.transactionCache);
			should.exist(ccsd.rawTransactionCache);
			should.exist(ccsd.rawJsonTransactionCache);
			should.exist(ccsd.blockCache);
			should.exist(ccsd.blockJsonCache);
			should.exist(ccsd.blockSubsidyCache);
			should.exist(ccsd.rawBlockCache);
			should.exist(ccsd.blockHeaderCache);
			should.exist(ccsd.zmqKnownTransactions);
			should.exist(ccsd.zmqKnownBlocks);
			should.exist(ccsd.lastTip);
			should.exist(ccsd.lastTipTimeout);
			should.equal(ccsd.dgpInfoCache, null, 'should be null');
			should.equal(ccsd.miningInfoCache, null, 'should be null');
			should.equal(ccsd.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(ccsd.maxTxids, 1000);
			should.equal(ccsd.maxTransactionHistory, 50);
			should.equal(ccsd.maxAddressesQuery, 10000);
			should.equal(ccsd.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(ccsd.spawnRestartTime, 5000);
			should.equal(ccsd.spawnStopTime, 10000);

			// try all interval
			should.equal(ccsd.tryAllInterval, 1000);
			should.equal(ccsd.startRetryInterval, 5000);

			// rpc limits
			should.equal(ccsd.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(ccsd.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var ccsd = new CcSService(baseConfig);
			ccsd.nodes.should.deep.equal([]);
			ccsd.nodesIndex.should.equal(0);
			ccsd.nodes.push({ client: sinon.stub() });
			should.exist(ccsd.client);
		});
		it('will set subscriptions', function() {
			var ccsd = new CcSService(baseConfig);
			ccsd.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var ccsd = new CcSService(baseConfig);
			ccsd._initDefaults({ transactionConcurrency: 10 });
			ccsd.transactionConcurrency.should.equal(10);
			ccsd._initDefaults({});
			ccsd.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			CcSService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var ccsd = new CcSService(baseConfig);
			var methods = ccsd.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var ccsd = new CcSService(baseConfig);
			var events = ccsd.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('ccsd/rawtransaction');
			events[0].scope.should.equal(ccsd);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('ccsd/hashblock');
			events[1].scope.should.equal(ccsd);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('ccsd/addresstxid');
			events[2].scope.should.equal(ccsd);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('ccsd/addressbalance');
			events[3].scope.should.equal(ccsd);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var ccsd = new CcSService(baseConfig);
			ccsd.subscribe = sinon.stub();
			ccsd.unsubscribe = sinon.stub();
			var events = ccsd.getPublishEvents();

			events[0].subscribe('test');
			ccsd.subscribe.args[0][0].should.equal('rawtransaction');
			ccsd.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			ccsd.unsubscribe.args[0][0].should.equal('rawtransaction');
			ccsd.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			ccsd.subscribe.args[1][0].should.equal('hashblock');
			ccsd.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			ccsd.unsubscribe.args[1][0].should.equal('hashblock');
			ccsd.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var ccsd = new CcSService(baseConfig);
			var emitter = {};
			ccsd.subscribe('hashblock', emitter);
			ccsd.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			ccsd.subscribe('rawtransaction', emitter2);
			ccsd.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			ccsd.subscribe('hashblock', emitter1);
			ccsd.subscribe('hashblock', emitter2);
			ccsd.subscribe('hashblock', emitter3);
			ccsd.subscribe('hashblock', emitter4);
			ccsd.subscribe('hashblock', emitter5);
			ccsd.subscriptions.hashblock.length.should.equal(5);

			ccsd.unsubscribe('hashblock', emitter3);
			ccsd.subscriptions.hashblock.length.should.equal(4);
			ccsd.subscriptions.hashblock[0].should.equal(emitter1);
			ccsd.subscriptions.hashblock[1].should.equal(emitter2);
			ccsd.subscriptions.hashblock[2].should.equal(emitter4);
			ccsd.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var ccsd = new CcSService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			ccsd.subscriptions.hashblock = [emitter1];
			ccsd.unsubscribe('hashblock', emitter3);
			ccsd.subscriptions.hashblock.length.should.equal(1);
			ccsd.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			ccsd.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(ccsd.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var ccsd = new CcSService(baseConfig);
			var emitter = new EventEmitter();
			ccsd.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var ccsd = new CcSService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ccsd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			ccsd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			ccsd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ccsd.unsubscribeAddress(emitter1);
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ccsd.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			ccsd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ccsd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ccsd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(ccsd, 'unsubscribeAddressAll');
			ccsd.unsubscribeAddress(emitter1);
			ccsd.unsubscribeAddressAll.callCount.should.equal(1);
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ccsd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ccsd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			ccsd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			ccsd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			ccsd.unsubscribeAddressAll(emitter1);
			ccsd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ccsd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			ccsd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(ccsd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var ccsd = new CcSService(baseConfig);
			var config = ccsd._getDefaultConfig();
			config.should.equal(defaultCcSConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a ccs.conf file', function() {
			var TestCcS = proxyquire('../../lib/services/ccsd', {
				fs: {
					readFileSync: readFileSync,
			...
			var ccsd = new TestCcS(baseConfig);
			ccsd.options.spawn.datadir = '/tmp/.ccs';

			sinon.spy(ccsd, '_expandRelativeDatadir');
			sinon.spy(ccsd, '_checkConfigIndexes');
			sinon.spy(ccsd, '_getDefaultConf')
			sinon.spy(ccsd, '_getNetworkConfigPath');
			sinon.spy(ccsd, '_parseBitcoinConf');

			var node = {};
			ccsd._loadSpawnConfiguration(node);

			ccsd._expandRelativeDatadir.callCount.should.equal(1);
			ccsd._checkConfigIndexes.callCount.should.equal(1);
			ccsd._getDefaultConf.callCount.should.equal(1);
			ccsd._getNetworkConfigPath.callCount.should.equal(1);
			ccsd._parseBitcoinConf.callCount.should.equal(2);

			should.exist(ccsd.spawn.config);
			ccsd.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: ccscore.Networks.testnet,
					configPath: '/tmp/.ccscore/ccscore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			ccsd.options.spawn.datadir = './data';
			var node = {};
			ccsd._loadSpawnConfiguration(node);
			...
			ccsd.options.spawn.datadir.should.equal('/tmp/.ccscore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestCcS = proxyquire('../../lib/services/ccsd', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badccs.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var ccsd = new TestCcS(baseConfig);
			(function() {
				ccsd._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(ccscore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the ccs.conf file', function() {
			var ccsd = new CcSService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				ccsd._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the ccs.conf file', function() {
			var ccsd = new CcSService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the ccs.conf file', function() {
			var ccsd = new CcSService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the ccs.conf file', function() {
			var ccsd = new CcSService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the ccs.conf file', function() {
			var ccsd = new CcSService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				ccsd.transactionDetailedCache.set(keys[i], {});
				ccsd.utxosCache.set(keys[i], {});
				ccsd.accountInfo.set(keys[i], {});
				ccsd.txidsCache.set(keys[i], {});
				ccsd.balanceCache.set(keys[i], {});
				ccsd.summaryCache.set(keys[i], {});
				ccsd.blockOverviewCache.set(keys[i], {});
				ccsd.blockJsonCache.set(keys[i], {});
			}
			ccsd._resetCaches();
			should.equal(ccsd.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(ccsd.utxosCache.get(keys[0]), undefined);
			should.equal(ccsd.accountInfo.get(keys[0]), undefined);
			should.equal(ccsd.txidsCache.get(keys[0]), undefined);
			should.equal(ccsd.balanceCache.get(keys[0]), undefined);
			should.equal(ccsd.summaryCache.get(keys[0]), undefined);
			should.equal(ccsd.blockOverviewCache.get(keys[0]), undefined);
			should.equal(ccsd.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			ccsd._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				ccsd.nodes[0].client.getInfo.callCount.should.equal(1);
				ccsd.nodes[1].client.getInfo.callCount.should.equal(1);
				ccsd.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			ccsd._tryAllClients(function(client, next) {
				...
				ccsd.nodes[1].client.getInfo.callCount.should.equal(1);
				ccsd.nodes[2].client.getInfo.callCount.should.equal(1);
				ccsd.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert ccsd-rpc error object into JavaScript error', function() {
			var ccsd = new CcSService(baseConfig);
			var error = ccsd._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			ccsd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			ccsd.nodes.push({
				client: {
			...
			ccsd._initChain(function() {
				log.info.callCount.should.equal(1);
				ccsd.getRawBlock.callCount.should.equal(1);
				ccsd.getRawBlock.args[0][0].should.equal('genesishash');
				ccsd.height.should.equal(5000);
				ccsd.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			ccsd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			ccsd._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			ccscore.Networks.disableRegtest();
			baseConfig.node.network = ccscore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: ccscore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var ccsd = new CcSService(config);
			ccsd._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var ccsd = new CcSService(config);
			ccsd._getDefaultConf().rpcport.should.equal(18332);
		});
		it('will get default rpc port for regtest', function() {
			ccscore.Networks.enableRegtest();
			var config = {
				node: {
					network: ccscore.Networks.testnet
				},
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var ccsd = new CcSService(config);
			should.equal(ccsd._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var ccsd = new CcSService(config);
			ccsd._getNetworkConfigPath().should.equal('testnet3/ccs.conf');
		});
		it('will get default rpc port for regtest', function() {
			ccscore.Networks.enableRegtest();
			var config = {
			...
			var ccsd = new CcSService(config);
			ccsd._getNetworkConfigPath().should.equal('regtest/ccs.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var ccsd = new CcSService(baseConfig);
			ccsd.node.network = ccscore.Networks.testnet;
			ccsd._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var ccsd = new CcSService(baseConfig);
			ccsd.node.network = ccscore.Networks.testnet;
			ccscore.Networks.enableRegtest();
			ccsd._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var ccsd = new CcSService(baseConfig);
			ccsd.node.network = ccscore.Networks.livenet;
			ccscore.Networks.enableRegtest();
			should.equal(ccsd._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			ccsd._rapidProtectedUpdateTip = sinon.stub();
			ccsd.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			ccsd._zmqBlockHandler(node, message);
			ccsd._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			ccsd.on('block', function(block) {
				block.should.equal(message);
				ccsd.emit.callCount.should.equal(1);
				ccsd._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			ccsd._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			ccsd._zmqBlockHandler(node, message);
			ccsd._rapidProtectedUpdateTip.callCount.should.equal(1);
			ccsd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			ccsd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(ccsd, 'emit');
			var emitter = new EventEmitter();
			ccsd.subscriptions.hashblock.push(emitter);
			emitter.on('ccsd/hashblock', function(blockHash) {
				ccsd._rapidProtectedUpdateTip.callCount.should.equal(1);
				ccsd.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				ccsd._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.syncPercentage = sinon.stub();
			sinon.spy(ccsd, '_resetCaches');
			ccsd.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				ccsd._resetCaches.callCount.should.equal(1);
			...
			ccsd._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			ccsd.on('synced', function() {
				ccsd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			ccsd.on('synced', function() {
				ccsd.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			ccsd.on('error', function(err) {
				log.error.callCount.should.equal(1);
				ccsd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.syncPercentage = sinon.stub();
			ccsd._resetCaches = sinon.stub();
			ccsd.on('tip', function(height) {
				ccsd._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				ccsd.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using ccscore.Transaction', function() {
			var ccsd = new CcSService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = ccscore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(ccscore.Networks.testnet);
			var outputAddress = ccscore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = ccscore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: ccscore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = ccsd._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(ccscore.Transaction.Output({
				script: ccscore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = ccsd._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var ccsd = new CcSService(baseConfig);
			var tx = ccscore.Transaction();
			var address = ccscore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(ccscore.Transaction.Output({
				script: ccscore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(ccscore.Transaction.Output({
				script: ccscore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			ccsd._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			ccsd.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('ccsd/addresstxid', function(data) {
			...
			ccsd._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			ccsd.subscriptions.rawtransaction.push(emitter);
			emitter.on('ccsd/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			ccsd._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(ccsd, 'emit');
			ccsd.subscriptions.rawtransaction.push(emitter);
			emitter.on('ccsd/rawtransaction', function() {
				done();
			});
			var node = {};
			ccsd._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd._updateTip = sinon.stub();
			ccsd._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			ccsd.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			ccsd._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				ccsd._updateTip.callCount.should.equal(11);
				ccsd._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				ccsd._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			ccsd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				ccsd._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			ccsd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				ccsd._zmqBlockHandler.callCount.should.equal(0);
				ccsd._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var ccsd = new CcSService(baseConfig);
			var node = {};
			ccsd._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			ccsd._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			ccsd._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			ccsd._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestCcSService = proxyquire('../../lib/services/ccsd', {
				fs: {
					readFile: readFile
				}
			});
			var ccsd = new TestCcSService(baseConfig);
			ccsd.spawnStopTime = 1;
			ccsd._process = {};
			ccsd._process.kill = sinon.stub();
			ccsd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				ccsd._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			ccsd._process.kill = sinon.stub().throws(error2);
			ccsd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd._loadSpawnConfiguration = sinon.stub();
			ccsd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			ccsd._spawnChildProcess(function(err) {
				ccsd._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var ccsd = new CcSService(baseConfig);
			ccsd._loadSpawnConfiguration = sinon.stub();
			ccsd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			ccsd._spawnChildProcess(function(err) {
				ccsd._loadSpawnConfiguration.callCount.should.equal(1);
				ccsd._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var ccsd = new TestCcSService(config);
			ccsd.spawn = {};
			ccsd._loadSpawnConfiguration = sinon.stub();
			ccsd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			ccsd.node.stopping = true;
			ccsd._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			ccsd._loadSpawnConfiguration = sinon.stub();
			ccsd.spawn = {};
			ccsd.spawn.exec = 'testexec';
			ccsd.spawn.configPath = 'testdir/ccs.conf';
			ccsd.spawn.datadir = 'testdir';
			ccsd.spawn.config = {};
			ccsd.spawn.config.rpcport = 20001;
			ccsd.spawn.config.rpcuser = 'ccs';
			ccsd.spawn.config.rpcpassword = 'password';
			ccsd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			ccsd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			ccsd._initZmqSubSocket = sinon.stub();
			ccsd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			ccsd._checkReindex = sinon.stub().callsArgWith(1, null);
			ccsd._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/ccs.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				ccsd._loadTipFromNode.callCount.should.equal(1);
				ccsd._initZmqSubSocket.callCount.should.equal(1);
				should.exist(ccsd._initZmqSubSocket.args[0][0].client);
				ccsd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				ccsd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(ccsd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn ccsd spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestCcSService = proxyquire('../../lib/services/ccsd', {
				fs: {
			...
			ccsd._loadSpawnConfiguration = sinon.stub();
			ccsd.spawn = {};
			ccsd.spawn.exec = 'ccsd';
			ccsd.spawn.datadir = '/tmp/ccs';
			ccsd.spawn.configPath = '/tmp/ccs/ccs.conf';
			ccsd.spawn.config = {};
			ccsd.spawnRestartTime = 1;
			ccsd._loadTipFromNode = sinon.stub().callsArg(1);
			ccsd._initZmqSubSocket = sinon.stub();
			ccsd._checkReindex = sinon.stub().callsArg(1);
			ccsd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			ccsd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(ccsd, '_spawnChildProcess');
			ccsd._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						ccsd._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			ccsd._spawnChildProcess(function(err) {
				...
				ccsd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				ccsd.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn ccsd spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestCcSService = proxyquire('../../lib/services/ccsd', {
				fs: {
			...
			ccsd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						ccsd._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			ccsd.spawn.config.rpcpassword = 'password';
			ccsd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			ccsd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			ccsd._spawnChildProcess(function(err) {
				ccsd._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			ccsd._initZmqSubSocket = sinon.stub();
			ccsd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			ccsd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var ccsd = new CcSService(config);
			ccsd.node.stopping = true;
			ccsd.startRetryInterval = 100;
			ccsd._loadTipFromNode = sinon.stub();
			ccsd._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to ccsd/);
				ccsd._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			ccsd.startRetryInterval = 1;
			var config = {};
			ccsd._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				ccsd._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			ccsd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			ccsd._connectProcess(config, function(err, node) {
				should.not.exist(err);
				ccsd._loadTipFromNode.callCount.should.equal(1);
				ccsd._initZmqSubSocket.callCount.should.equal(1);
				ccsd._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.options = {};
			ccsd.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('CcS configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			ccsd.options = {
				connect: [
					{}
				]
			...
			ccsd.start(function(err) {
				ccsd._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var ccsd = new CcSService(baseConfig);
			var node = {};
			ccsd._initChain = sinon.stub().callsArg(0);
			ccsd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			ccsd.options = {
				spawn: {}
			};
			ccsd.start(function(err) {
				should.not.exist(err);
				ccsd.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(ccsd, '_spawnChildProcess');
			var nodes = [{}];
			ccsd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			ccsd.options = {
				connect: [
					{}
				]
			...
			ccsd.start(function(err) {
				should.not.exist(err);
				ccsd._initChain.callCount.should.equal(1);
				ccsd._connectProcess.callCount.should.equal(1);
				ccsd.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			ccsd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				ccsd.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			ccsd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			ccsd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			ccsd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.syncPercentage(function(err) {
				should.exist(err);
				ccsd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			ccsd.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				ccsd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var ccsd = new CcSService(baseConfig);
			var args = ccsd._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var ccsd = new CcSService(baseConfig);
			var args = ccsd._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var ccsd = new CcSService(baseConfig);
			sinon.spy(ccsd, '_normalizeAddressArg');
			ccsd.nodes.push({
				client: {
			...
			var options = {};
			ccsd.getAddressBalance(address, options, function(err) {
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			ccsd.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				ccsd.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					ccsd._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			ccsd.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = ccsd._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = ccsd._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = ccsd._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = ccsd._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				ccsd._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			ccsd.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(ccsd, '_getHeightRangeQuery');
			sinon.spy(ccsd, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			ccsd.getAddressTxids(address, options, function(err) {
				should.exist(err);
				ccsd._getHeightRangeQuery.callCount.should.equal(1);
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			ccsd.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				ccsd._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			ccsd.getAddressTxids(address, options, function(err, txids) {
				...
				ccsd.getAddressTxids(address, options, function(err, txids) {
					...
					ccsd._getHeightRangeQuery.callCount.should.equal(2);
					ccsd._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			ccsd.getAddressTxids(address, options, function(err, txids) {
				...
				ccsd.getAddressTxids(address, options, function(err, txids) {
					...
					ccsd._getHeightRangeQuery.callCount.should.equal(4);
					ccsd._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			ccsd.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				ccsd.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					ccsd.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						ccsd._getHeightRangeQuery.callCount.should.equal(3);
						ccsd._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var ccsd = new CcSService(baseConfig);
			ccsd.height = 10;
			var confirmations = ccsd._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var ccsd = new CcSService(baseConfig);
			var result = {};
			ccsd._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var ccsd = new CcSService(baseConfig);
			var result = {};
			ccsd._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var ccsd = new CcSService(baseConfig);
			var result = {};
			ccsd._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var ccsd = new CcSService(baseConfig);
			var result = {};
			ccsd._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(ccsd, '_getAddressDetailsForInput');
			sinon.spy(ccsd, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = ccsd._getAddressDetailsForTransaction(tx, addresses);
			...
			ccsd._getAddressDetailsForInput.callCount.should.equal(1);
			ccsd._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var ccsd = new CcSService(baseConfig);
			sinon.spy(ccsd, '_getConfirmationsDetail');
			ccsd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			ccsd.height = 300;
			var addresses = {};
			ccsd._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			ccsd._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				ccsd.getDetailedTransaction.callCount.should.equal(1);
				ccsd._getAddressDetailsForTransaction.callCount.should.equal(1);
				ccsd._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var ccsd = new CcSService(baseConfig);
			ccsd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			ccsd._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				ccsd.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from ccscore addresses', function() {
			var addresses = [
				ccscore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				ccscore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var ccsd = new CcSService(baseConfig);
			var strings = ccsd._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var ccsd = new CcSService(baseConfig);
			(function() {
				ccsd._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var ccsd = new CcSService(baseConfig);
			sinon.spy(ccsd, '_normalizeAddressArg');
			ccsd.maxAddressesQuery = 100;
			ccsd.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				ccsd._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var ccsd = new CcSService(baseConfig);
			sinon.spy(ccsd, '_normalizeAddressArg');
			sinon.spy(ccsd, '_getAddressStrings');
			ccsd.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				ccsd._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(ccsd, '_normalizeAddressArg');
			sinon.spy(ccsd, '_getAddressStrings');
			ccsd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			ccsd.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				ccsd._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(ccsd, '_normalizeAddressArg');
			sinon.spy(ccsd, '_getAddressStrings');
			ccsd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			ccsd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			ccsd.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				ccsd._getAddressStrings.callCount.should.equal(1);
				ccsd.getAddressTxids.callCount.should.equal(1);
				ccsd._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(ccsd, '_normalizeAddressArg');
			sinon.spy(ccsd, '_getAddressStrings');
			sinon.spy(ccsd, 'getAddressTxids');
			ccsd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			ccsd.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				ccsd._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(ccsd, '_normalizeAddressArg');
			sinon.spy(ccsd, '_getAddressStrings');
			sinon.spy(ccsd, '_paginateTxids');
			ccsd._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(ccsd, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			ccsd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			ccsd.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				ccsd._getAddressStrings.callCount.should.equal(1);
				ccsd.getAddressTxids.callCount.should.equal(1);
				ccsd._getAddressDetailedTransaction.callCount.should.equal(2);
				ccsd._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			ccsd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			ccsd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			ccsd.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			ccsd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			ccsd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			ccsd.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			ccsd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			ccsd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			ccsd.getAddressSummary(address, options, function(err, summary) {
				ccsd._paginateTxids.callCount.should.equal(1);
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				ccsd.getAddressTxids.callCount.should.equal(1);
				ccsd.getAddressBalance.callCount.should.equal(1);
				ccsd._paginateTxids.args[0][1].should.equal(0);
				ccsd._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			ccsd.getAddressSummary(address, options, function(err, summary) {
				...
				ccsd.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					ccsd._paginateTxids.callCount.should.equal(0);
					ccsd._normalizeAddressArg.callCount.should.equal(2);
					ccsd.getAddressTxids.callCount.should.equal(1);
					ccsd.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			ccsd.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				ccsd._paginateTxids.callCount.should.equal(1);
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				ccsd.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			ccsd._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			ccsd.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(ccsd, '_tryAllClients');
			ccsd.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			ccsd.getRawBlock(blockhash, function(err) {
				should.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			ccsd.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			ccsd.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			ccsd.getBlock(0, function(err) {
				ccsd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as ccscore object from height', function(done) {
			var ccsd = new CcSService(baseConfig);
			sinon.spy(ccsd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			ccsd.getBlock(0, function(err, block) {
				should.not.exist(err);
				ccsd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(ccscore.Block);
				done();
			});
		});
		it('will getblock as ccscore object', function(done) {
			var ccsd = new CcSService(baseConfig);
			sinon.spy(ccsd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			ccsd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			ccsd.getBlock(hash, function(err, block) {
				should.not.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			ccsd.getBlock(0, function(err, block) {
				...
				ccsd.getBlock(0, function(err, block) {
					should.not.exist(err);
					ccsd._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			ccsd.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var ccsd = new CcSService(baseConfig);
			sinon.spy(ccsd, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			ccsd.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			ccsd.getBlockHeader(10, function(err) {
				ccsd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			ccsd.getBlockHeader(blockhash, function(err) {
				ccsd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			ccsd.getBlockHeader(0, function(err) {
				ccsd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			ccsd.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				ccsd._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			ccsd.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				ccsd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			ccsd._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				ccsd._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			ccsd._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			ccsd._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var ccsd = new CcSService(baseConfig);
			sinon.spy(ccsd, '_maybeGetBlockHash');
			ccsd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			ccsd.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				ccsd._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			ccsd.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				ccsd.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					ccsd._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			ccsd.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = ccscore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			ccsd.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			ccsd.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			ccsd.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = ccscore.Transaction();
			(function() {
				ccsd.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getRawTransaction('txid', function(err) {
				should.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			ccsd.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getTransaction('txid', function(err) {
				should.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			ccsd.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				ccsd._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(ccscore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			ccsd.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				ccsd._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			ccsd.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			ccsd.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			ccsd.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.node.getNetworkName = sinon.stub().returns('testnet');
			...
			ccsd.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			ccsd.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			ccsd.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				ccsd.getSubsidy.callCount.should.equal(1);
				ccsd.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			ccsd.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.spawn = {};
			ccsd.spawn.process = new EventEmitter();
			ccsd.spawn.process.kill = sinon.stub();
			ccsd.stop(done);
			ccsd.spawn.process.kill.callCount.should.equal(1);
			ccsd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			ccsd.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			ccsd.spawn.process.kill.callCount.should.equal(1);
			ccsd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			ccsd.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var ccsd = new CcSService(baseConfig);
			ccsd.shutdownTimeout = 300;
			ccsd.spawn = {};
			ccsd.spawn.process = new EventEmitter();
			ccsd.spawn.process.kill = sinon.stub();
			ccsd.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var ccsd = new CcSService(baseConfig);
			sinon.spy(ccsd, '_normalizeAddressArg');
			sinon.spy(ccsd, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			ccsd.nodes.push({
			...
			ccsd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				ccsd._getBalanceFromMempool.callCount.should.equal(1);
				ccsd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			ccsd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			ccsd.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			ccsd.getJsonBlock(0, function(err) {
				should.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			ccsd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			ccsd.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				ccsd._tryAllClients.callCount.should.equal(2);
				ccsd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			ccsd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
				ccsd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var ccsd = new CcSService(baseConfig);
			var emitter = new EventEmitter();
			ccsd.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(ccsd.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var ccsd = new CcSService(baseConfig);
			var emitter = new EventEmitter();
			ccsd.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ccsd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			ccsd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			ccsd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ccsd.unsubscribeBalance(emitter1);
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ccsd.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			ccsd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var ccsd = new CcSService(baseConfig);
			...
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ccsd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			ccsd.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var ccsd = new CcSService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ccsd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(ccsd, 'unsubscribeBalanceAll');
			ccsd.unsubscribeBalance(emitter1);
			ccsd.unsubscribeBalanceAll.callCount.should.equal(1);
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ccsd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			ccsd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			ccsd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			ccsd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			ccsd.unsubscribeBalanceAll(emitter1);
			ccsd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			ccsd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			ccsd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(ccsd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var ccsd = new CcSService(baseConfig);
			sinon.spy(ccsd, '_notifyBalanceSubscriber');
			ccsd._getAddressesFromTransaction = sinon.stub().returns(addresses);
			ccsd.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				ccsd._notifyBalanceSubscriber.callCount.should.equal(1);
				ccsd._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			ccsd._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				ccsd.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			ccsd._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			ccsd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getDgpInfo(function(err, result) {
				should.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getMiningInfo(function(err, result) {
				should.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			ccsd.getStakingInfo(function(err, result) {
				should.exist(err);
				ccsd._tryAllClients.callCount.should.equal(1);
